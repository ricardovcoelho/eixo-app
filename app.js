...
function wireSaveHandlers(){
  ...
  document.getElementById('btn-save-task').addEventListener('click',async()=>{
    ...
    const isAfazer=document.getElementById('task-modal-title').textContent.includes('Afazer');
    const timeData=isAfazer?{time:document.getElementById('t-time').value||null}:{time_start:document.getElementById('t-time-start').value||null,time_end:document.getElementById('t-time-end').value||null};
    if(editTaskId){const t=state.tasks.find(x=>x.id===editTaskId);if(t){t.name=name;t.due_date=document.getElementById('t-date').value||null;t.objective_id=objId;t.kr_id=krId;Object.assign(t,timeData);}await sbUpdate('tasks',editTaskId,{name,due_date:document.getElementById('t-date').value||null,objective_id:objId,kr_id:krId,...timeData});}
    else{const nt=await sbInsert('tasks',{name,due_date:document.getElementById('t-date').value||null,done:false,objective_id:objId,kr_id:krId,urg:0,imp:0,...timeData});if(nt&&!nt.error)state.tasks.push(nt);}
  ...
  document.getElementById('btn-save-routine').addEventListener('click',async()=>{
    ...
    const time=document.getElementById('r-time').value||null,startDate=document.getElementById('r-start').value||null,data={name,category:cat,frequency:freq,day_of_week:dow,time,start_date:startDate};
...
function openNewTask(objId,krId){editTaskId=null;document.getElementById('task-modal-title').textContent='Nova Tarefa';document.getElementById('t-name').value='';document.getElementById('t-date').value='';document.getElementById('t-time').style.display='none';document.getElementById('t-time-end-group').style.display='block';document.getElementById('t-time-start').value='';document.getElementById('t-time-end').value='';document.getElementById('t-obj').innerHTML='<option value="">— tarefa avulsa —</option>'+state.objectives.map(o=>'<option value="'+o.id+'"'+(o.id===objId?' selected':'')+'>'+o.name+'</option>').join('');const krSel=document.getElementById('t-kr'),krGrp=document.getElementById('t-kr-group');krSel.innerHTML='<option value="">— selecione o KR —</option>';if(objId){const o=state.objectives.find(x=>x.id===objId);if(o)o.krs.forEach(k=>{krSel.innerHTML+='<option value="'+k.id+'"'+(k.id===krId?' selected':'')+'>'+k.name+'</option>';});krGrp.style.display='block';}else krGrp.style.display='none';openModal('modal-task');}
function openEditTask(id){const t=state.tasks.find(x=>x.id===id);if(!t)return;editTaskId=id;document.getElementById('task-modal-title').textContent='Editar Tarefa';document.getElementById('t-name').value=t.name;document.getElementById('t-date').value=t.due_date||'';document.getElementById('t-time').style.display='none';document.getElementById('t-time-end-group').style.display='block';document.getElementById('t-time-start').value=t.time_start||'';document.getElementById('t-time-end').value=t.time_end||'';document.getElementById('t-obj').innerHTML='<option value="">— tarefa avulsa —</option>'+state.objectives.map(o=>'<option value="'+o.id+'"'+(o.id===t.objective_id?' selected':'')+'>'+o.name+'</option>').join('');const krSel=document.getElementById('t-kr'),krGrp=document.getElementById('t-kr-group');krSel.innerHTML='<option value="">— selecione o KR —</option>';if(t.objective_id){const o=state.objectives.find(x=>x.id===t.objective_id);if(o){o.krs.forEach(k=>{krSel.innerHTML+='<option value="'+k.id+'"'+(k.id===t.kr_id?' selected':'')+'>'+k.name+'</option>';});krGrp.style.display='block';}}else krGrp.style.display='none';openModal('modal-task');}
function openNovoAfazer(){editTaskId=null;document.getElementById('task-modal-title').textContent='Novo Afazer';document.getElementById('t-name').value='';document.getElementById('t-date').value='';document.getElementById('t-time').style.display='block';document.getElementById('t-time').value='';document.getElementById('t-time-end-group').style.display='none';document.getElementById('t-obj').innerHTML='<option value="">— sem vínculo —</option>';document.getElementById('t-kr-group').style.display='none';openModal('modal-task');}
function openEditAfazer(id){const t=state.tasks.find(x=>x.id===id);if(!t)return;editTaskId=id;document.getElementById('task-modal-title').textContent='Editar Afazer';document.getElementById('t-name').value=t.name;document.getElementById('t-date').value=t.due_date||'';document.getElementById('t-time').style.display='block';document.getElementById('t-time').value=t.time||'';document.getElementById('t-time-end-group').style.display='none';document.getElementById('t-obj').innerHTML='<option value="">— sem vínculo —</option>';document.getElementById('t-kr-group').style.display='none';openModal('modal-task');}
...
// Reconstrói as opções do select de categoria do formulário de rotina,
// incluindo os grupos customizados (senão "Salvar" grava numa categoria errada)
function populateRCatOptions(){
  ...
  sel.innerHTML=cats.map(function(c){return '<option value="'+c.key+'">'+c.label+'</option>';}).join('');
}
...
function getEventsForDate(dateStr){
  ...
  state.routines.forEach(function(r){
    if(r.start_date&&r.start_date>dateStr)return; // ainda não iniciada nessa data
    ...
    else if(freq==='weekly')show=dow===5;
    else if(freq==='custom_day')show=dowList.indexOf(dow)!==-1;
    else if(freq==='monthly'){var lf=new Date(dt.getFullYear(),dt.getMonth()+1,0);while(lf.getDay()!==5)lf.setDate(lf.getDate()-1);show=dom===lf.getDate();}
...
function gerarRelatorio(){
  ...
  var secDash=document.getElementById('rel-dashboard').checked;
  var secProj=document.getElementById('rel-projetos').checked;
  var secRot=document.getElementById('rel-rotinas').checked;
  var secAfaz=document.getElementById('rel-afazeres').checked;
  var secAgenda=document.getElementById('rel-agenda').checked;
  var secMatriz=document.getElementById('rel-matriz').checked;
  var now=new Date();
  var periodoLabel={mes:'Este mês ('+now.toLocaleString('pt-BR',{month:'long',year:'numeric'})+')',semana:'Esta semana',hoje:'Hoje ('+now.toLocaleDateString('pt-BR')+')',tudo:'Completo'}[periodo];
  var dtIni=null,dtFim=new Date();dtFim.setHours(23,59,59,999);
  if(periodo==='hoje'){dtIni=new Date();dtIni.setHours(0,0,0,0);}
  else if(periodo==='semana'){dtIni=new Date();dtIni.setDate(now.getDate()-now.getDay()+1);dtIni.setHours(0,0,0,0);}
  else if(periodo==='mes'){dtIni=new Date(now.getFullYear(),now.getMonth(),1);}
  function inPeriod(d){if(!d||!dtIni)return true;var dt=new Date(d+'T00:00:00');return dt>=dtIni&&dt<=dtFim;}
  var nome=currentUser&&currentUser.name?currentUser.name:'Usuário';
  var h='<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><title>Relatório Good Day</title>'
    +'<style>body{font-family:Arial,sans-serif;color:#1B1B3A;max-width:900px;margin:0 auto;padding:32px;font-size:13px}'
    +'h1{color:#1B1B3A;font-size:24px;margin-bottom:4px}h2{color:#C65D3B;font-size:16px;margin:28px 0 12px;border-bottom:2px solid #C65D3B;padding-bottom:6px}'
    +'h3{font-size:14px;color:#355F79;margin:16px 0 8px}'
    +'.header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px;padding-bottom:16px;border-bottom:3px solid #1B1B3A}'
    +'.badge{display:inline-block;padding:2px 8px;border-radius:99px;font-size:11px;font-weight:700;margin-right:4px}'
    +'.badge-red{background:#fde8e5;color:#C0392B}.badge-green{background:#e5f5ed;color:#2E7D52}.badge-gray{background:#f0f0f0;color:#666}'
    +'.metric-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:16px}'
    +'.metric-card{border:1px solid #eee;border-radius:8px;padding:12px;text-align:center}'
    +'.metric-val{font-size:28px;font-weight:800;color:#1B1B3A}.metric-lbl{font-size:10px;color:#999;text-transform:uppercase;letter-spacing:1px}'
    +'.progress{height:6px;background:#eee;border-radius:3px;margin:6px 0}'
    +'.progress-fill{height:100%;border-radius:3px;background:#C65D3B}'
    +'table{width:100%;border-collapse:collapse;margin-bottom:12px}'
    +'th{background:#1B1B3A;color:#fff;padding:8px 10px;text-align:left;font-size:12px}'
    +'td{padding:7px 10px;border-bottom:1px solid #eee;font-size:12px}'
    +'tr:nth-child(even)td{background:#f9f9f9}'
    +'.done{text-decoration:line-through;color:#999}.overdue{color:#C0392B;font-weight:600}'
    +'@media print{body{padding:16px}}</style></head><body>';
  h+='<div class="header"><div><h1>📊 Relatório Good Day</h1>'
    +'<div style="color:#666;font-size:12px">'+nome+' · Gerado em '+now.toLocaleDateString('pt-BR')+' às '+now.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})+'</div></div>'
    +'<div style="text-align:right"><div style="font-weight:700;color:#C65D3B">'+periodoLabel+'</div></div></div>';
  if(secDash){
    var taskDone=state.tasks.filter(function(t){return t.done;}).length;
    var taskTotal=state.tasks.length;
  ...
  if(secAfaz){
    var afazeres=state.tasks.filter(function(t){return !t.objective_id;});
    var afazPeriod=afazeres.filter(function(t){return inPeriod(t.due_date);});
    if(!afazPeriod.length)afazPeriod=afazeres;
    h+='<h2>⚡ Afazeres</h2><table><tr><th>Descrição</th><th>Prazo</th><th>Horário</th><th>Status</th></tr>';
    afazPeriod.forEach(function(t){var ov=!t.done&&isOverdue(t.due_date);h+='<tr><td class="'+(t.done?'done':ov?'overdue':'')+'">'+(ov?'⚠ ':'')+t.name+'</td><td>'+(t.due_date||'—')+'</td><td>'+(t.time||'—')+'</td><td>'+(t.done?'<span class="badge badge-green">✓</span>':ov?'<span class="badge badge-red">Atrasado</span>':'<span class="badge badge-gray">Pendente</span>')+'</td></tr>';});
    h+='</table>';
  }
  if(secAgenda){
...
function renderCascataObj(o){
  ...
  h += '<div style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:1px">Resultados-Chave</div>';
  h += '<div style="display:flex;gap:6px">';
  h += '<button class="btn btn-sm btn-novo-kr-this" data-objid="'+o.id+'">+ Novo Resultado-Chave</button>';
  h += '<button class="btn btn-sm btn-accent btn-nova-tarefa-this" data-objid="'+o.id+'">+ Tarefa</button>';
  h += '</div></div>';

  if(!o.krs || !o.krs.length){
    h += '<div style="color:var(--text3);padding:14px;text-align:center;background:var(--bg2);border:1px dashed var(--border2);border-radius:var(--radius);font-size:12px">Nenhum Resultado-Chave ainda.</div>';
  } else {
...
function wireCascataAll(el){
  ...
  // + Novo Resultado-Chave (dentro do objetivo)
      ...
      body: `
        <div class="ob-cascade-demo">
          ...
          <div class="ob-cascade-item ob-level-3">
            ...
            <div><div class="ob-cascade-label">RESULTADO-CHAVE</div><div class="ob-cascade-name">Treinar 4x por semana</div></div>
          </div>
          <div class="ob-cascade-arrow">↓</div>
